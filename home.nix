...

{
  ...
  programs.zsh = {
    ...
    initExtra = ''
      ...
      # Enhanced file extraction with business document support
      extract() {
        if [[ -f "$1" ]]; then
          case "$1" in
            *.tar.gz)  tar -xzf "$1" ;;
      ...
      # Quick search and replace for business documents
      sr() {
        (( $# != 3 )) && { echo "Usage: sr <search> <replace> <file>"; return 1; }
        sed -i "s/$1/$2/g" "$3"
      }
      
      # Fuzzy directory navigation with business context
      fd() {
        local dir
