# -----------------------------------------------------------------------------
# UPDATED: hosts/server/config.nix (SYSTEM LEVEL - CLEANED)
# -----------------------------------------------------------------------------
{ config, pkgs, lib, ... }:

{
  ...
  ####################################################################
  # 5. ZSH CONFIGURATION - SYSTEM LEVEL
  ####################################################################
  programs.zsh = {
    enable = true;
  ...
  ####################################################################
  # 8. USER GROUPS & PERMISSIONS
  ####################################################################
  users.users.eric.extraGroups = [ 
    "wheel" "networkmanager" "video" "audio" "docker" "podman" 
  ];
  ...
  ####################################################################
  # 11. SERVER SYSTEM PACKAGES - FIXED SYNTAX
  ####################################################################
