...
EOF
  ...
  ####################################################################
        ...
        systemd.services.slskd-config-seeder = {
          description = "Seed slskd config.yml if it does not exist";
          ...
          script = ''
            ...
            if [ ! -f "$CONFIG_FILE" ]; then
              echo "Creating boilerplate slskd.yml in $CONFIG_DIR"
              cat > "$CONFIG_FILE" <<EOF
      # Boilerplate configuration generated by NixOS
      slsk:
        username: "eriqueok"
        password: "il0wwlm?"
        shared_dirs:
      ...
      downloads:
        path: /downloads/music/complete
        incomplete_path: /downloads/music/incomplete

      api:
        # The '$$' escapes the dollar sign for Nix, so it becomes a single '$' in the final script.
        key: "$${SLSKD_API_KEY:-}"
      ...
      EOF
              ...
              echo "slskd.yml already exists, skipping creation."
            fi
          '';
        };
